...
<%--1. defer сообщает браузеру, что он должен продолжать обрабатывать страницу и загружать скрипт в фоновом режиме, а затем запустить его, когда скрипт загрузится.
    2. скрипты с defer никогда не блокируют страницу и выполняются, когда дерево DOM готово, но до события DOMContentLoaded
    3. скрипты с defer сохраняют порядок относительно друг друга, как и обычные скрипты
    4. defer предназначен только для внешних скриптов и будет проигнорирован, если в теге <script> нет src--%>
        ...
        <table class="table" id="datatable">
            ...
            <c:forEach items="${users}" var="user">
                ...
                <tr id="${user.id}" class="${!user.enabled ? 'disabled_user' : ''}">
                    ...
                    <td><a class="delete"><span class="fa fa-remove" onclick=""></span></a></td>
                </tr>
            </c:forEach>
        </table>
    </div>
